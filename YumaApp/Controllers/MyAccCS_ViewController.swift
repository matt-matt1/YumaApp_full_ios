...
//		layout.sectionInset = UIEdgeInsets(top: 20, left: 8, bottom: 20, right: 8)
	...
	let errorMsg: UILabel =
	{
		...
		view.text = R.string.noCreditSlips
...
//		stackPanel.bottomAnchor.constraint(equalTo: viewOuter.bottomAnchor, constant: -5).isActive = true
	...
	fileprivate func setCollection()
	{
		if store.creditSlips.count > 0
		{
			...
			myCollection.register(MyAccCS_Cell.self, forCellWithReuseIdentifier: cellId)
		...
		else
		{
			stackWindow.constraints.forEach { (constraint) in
				if #available(iOS 10.0, *)
				{
					...
					myCollectionHeight = 40 + errorMsg.font.pointSize
					stackWindow.heightAnchor.constraint(equalToConstant: myCollectionHeight).isActive = true
				}
				else
				{
			...
			errorView2.topAnchor.constraint(equalTo: errorView.topAnchor, constant: 8).isActive = true
			errorView2.leadingAnchor.constraint(equalTo: errorView.leadingAnchor, constant: 15).isActive = true
			errorView2.bottomAnchor.constraint(equalTo: errorView.bottomAnchor, constant: -8).isActive = true
			errorView2.trailingAnchor.constraint(equalTo: errorView.trailingAnchor, constant: -15).isActive = true
