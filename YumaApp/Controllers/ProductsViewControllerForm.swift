...
extension ProductsViewController
{
	...
	func formTagsView2(tagIDs: [IdAsString]/*, maxWidth: CGFloat, maxHeight: CGFloat*/) //-> UICollectionView
	{
		...
		collectionTags.delegate = self //as? UICollectionViewDelegate
		collectionTags.dataSource = self //as? UICollectionViewDataSource
					...
					DispatchQueue.main.async()
						{
							...
							imageInFrame.addTapGestureRecognizer {
								print("copy this thumbnail to main image")
							}
			...
			if find < store.categories.count
			{
				for cat in store.categories
				{
					if cat.id == find
					{
						...
						wrapperLbl.addTapGestureRecognizer {
							print("filter list to this category")
						}
	...
	func makeTag(string: String) -> UIView
	{
		...
		tagView.addTapGestureRecognizer {
			print("filter list to this tag")
		}
		return tagView
	}
